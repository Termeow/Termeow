...
- `.github/workflows/snapshot.yml` publishes an ad-hoc zip to the moving `nightly` prerelease once a day (00:00 Asia/Shanghai) or on `workflow_dispatch`. That tag is not a SemVer release; keep `v*` tags on `main`.
