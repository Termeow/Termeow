...
Termeow is a **native macOS SSH client**. Capability ideas come from the original Termeow brief, plus [EdgeTerm](https://github.com/miskin-lee/EdgeTerm) and [VelaShell](https://github.com/joesdu/VelaShell). Do not port those stacks (Tauri, .NET, Avalonia). Copy the *job*, not the implementation.
...
- [ ] Import `~/.ssh/config`
- [ ] Import EdgeTerm / Xshell / WinSCP / other session files
...
- [ ] Encoding besides UTF-8 (GB18030 / GBK / Big5 / Shift_JIS — EdgeTerm)
- [ ] Locale / `LANG` on the SSH shell request
- [ ] Line-number and timestamp gutter (EdgeTerm / WindTerm)
- [ ] Block / multi-range selection
- [ ] Mouse reporting beyond what SwiftTerm already does
- [ ] Bell (visual / sound)
- [ ] Scrollback size cap in Settings
- [ ] Command suggestions from typed history (EdgeTerm)
- [ ] Sender / snippet bar: send text or hex to one tab or all tabs (EdgeTerm)
- [ ] Command palette (VelaShell)
...
- [ ] Reorder tabs
- [ ] Pin tab / tab color
- [ ] Background-tab activity / dirty highlight (EdgeTerm)
...
- [ ] Docked filer in the main window (EdgeTerm `⌘J`)
...
## Later (reference, not next)

These are real EdgeTerm / VelaShell features. Do not start them until SSH, SFTP, Agent, and a first release are solid.
