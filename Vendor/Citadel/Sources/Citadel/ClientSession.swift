...
final class ClientHandshakeHandler: ChannelInboundHandler, @unchecked Sendable {
    ...
    // Accessed only on the channel's event loop.
    ...
    init(eventLoop: EventLoop, loginTimeout: TimeAmount) {
        ...
        timeout = eventLoop.scheduleTask(in: loginTimeout) { [weak self] in
            self?.finish(.failure(ChannelError.connectTimeout(loginTimeout)))
        }
    }
    ...
    func channelInactive(context: ChannelHandlerContext) {
        finish(.failure(ChannelError.ioOnClosedChannel))
...
final class SSHClientSession: Sendable {
    ...
    static func addHandlers(
        ...
        return addHandlers(on: channel, inboundChannelHandler: inboundChannelHandler, settings: settings)
    }
    ...
    static func addHandlers(
        ...
        do {
            ...
            handshakeHandler.finish(.failure(error))
