...
extension SSHClient {
    ...
    internal func _executeCommandStream(
        environment: [SSHChannelRequestEvent.EnvironmentRequest] = [],
        mode: CommandMode,
        setupTimeout: TimeAmount = .seconds(15)
        ...
        let setup = SSHChannelSetup<Channel>(eventLoop: eventLoop)
        do {
            let channel = try await setup.run(timeout: setupTimeout) { [eventLoop, sshHandler = session.sshHandler] setup in
                let requests = SSHChannelRequestHandler()
                let createChannel = eventLoop.makePromise(of: Channel.self)
                sshHandler.value.createChannel(createChannel) { channel, _ in
                    guard setup.register(channel) else {
                        return eventLoop.makeFailedFuture(CancellationError())
                    }
                    return channel.pipeline.addHandlers(requests, handler)
                }
                return createChannel.futureResult.flatMap { channel in
                    var ready = eventLoop.makeSucceededVoidFuture()
                    for env in environment {
                        ready = ready.flatMap { requests.request(env, wantReply: env.wantReply, on: channel) }
                    }
                    switch mode {
                    case .pty(let request):
                        ready = ready.flatMap { requests.request(request, wantReply: request.wantReply, on: channel) }
                        fallthrough
                    case .tty:
                        ready = ready.flatMap {
                            requests.request(SSHChannelRequestEvent.ShellRequest(wantReply: true), on: channel)
                        }
                    case .command(let command):
                        ready = ready.flatMap {
                            requests.request(SSHChannelRequestEvent.ExecRequest(command: command, wantReply: true), on: channel)
                        }
                    }
                    if case .tty(.some(let command)) = mode {
                        ready = ready.flatMap {
                            channel.writeAndFlush(SSHChannelData(
                                type: .channel, data: .byteBuffer(ByteBuffer(string: command + ";exit\n"))
                            ))
                        }
                    }
                    return ready.map { channel }
                }
            }
    ...
    ///   - setupTimeout: Deadline for opening the channel and receiving PTY/shell acknowledgements, not the terminal lifetime.
