...
extension SSHClient {
    ...
    ///   - setupTimeout: Deadline for channel creation, subsystem acknowledgement, and version negotiation.
    ...
    public func openSFTP(
        ...
        let setup = SSHChannelSetup<SFTPClient>(eventLoop: eventLoop)
        return try await setup.run(timeout: setupTimeout) { [eventLoop, sshHandler = session.sshHandler] setup in
            ...
            createChannel.futureResult.whenFailure { createClient.fail($0) }
            ...
            sshHandler.value.createChannel(createChannel) { channel, _ in
                ...
                return channel.pipeline.addHandler(requests).flatMap {
                    SFTPClient.setupChannelHanders(channel: channel, logger: logger)
                }.map { createClient.succeed($0) }
            }
            ...
            return createChannel.futureResult.flatMap { channel in
                ...
                logger.debug("SFTP subsystem acknowledged")
